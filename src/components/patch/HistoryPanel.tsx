    ...
    <aside className="w-full h-full bg-card/40 backdrop-blur-xl flex flex-col border-l border-border">
      <div className="px-6 py-4 border-b border-border flex items-center gap-2">
        ...
        {history.length > 0 && onResetAll && (
          <button
            onClick={onResetAll}
            className="ml-2 flex items-center gap-1 text-[9px] px-2 py-0.5 rounded border border-border text-muted-foreground hover:text-foreground hover:border-foreground/20 transition-colors duration-150 font-medium uppercase tracking-wider"
          >
            <RotateCw size={9} />
            Reset All
          </button>
          ...
          {hasTrack && history.length > 0 ? (
            history.map((item, index) => (
              <motion.div
                ...
                className="p-3.5 rounded-lg panel-surface space-y-2 hover:bg-muted/20 transition-colors duration-150"
              >
